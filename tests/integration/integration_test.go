...
func setupTestEnvironment(t *testing.T) (*inference.Engine, *api.Server) {
	...
	// Use real TinyLlama tokenizer
	// Get project root from test file location (tests/integration/ -> project root)
	_, testFile, _, _ := runtime.Caller(0)
	projectRoot := filepath.Dir(filepath.Dir(filepath.Dir(testFile)))
	tokenizerPath := filepath.Join(projectRoot, "models", "tinyllama")
